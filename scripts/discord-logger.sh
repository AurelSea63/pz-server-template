...
case "${1}" in
    ...
    connect)    [ "${LOG_PLAYER_CONNECT:-1}" = "1" ]    && log_discord "👤 Connexion"          "**$2** a rejoint le serveur"   "65280" ;;
    disconnect) [ "${LOG_PLAYER_DISCONNECT:-1}" = "1" ] && log_discord "👤 Déconnexion"        "**$2** a quitté le serveur"    "16776960" ;;
